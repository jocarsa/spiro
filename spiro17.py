...
for i in range(0, 25):
    ...
    # Randomly set the number of arms (N) to a value between 2 and 4
    ...
    angles = np.zeros(N)
    radii = np.random.uniform(0, height // 3, N) * 1.5
    speeds = np.random.uniform(-2.5, 2.5, N) / 20
    ...
    arms_canvas = np.ones((height, width, 3), dtype=np.uint8) * 255   # To draw arms
    ...
    try:
        while frame_count < max_frames:
            ...
            if drawing_started:
                cv2.line(trace_canvas, (previous_x, previous_y), (x_current, y_current), trace_color, 225, cv2.LINE_AA)  # Anti-aliased line
